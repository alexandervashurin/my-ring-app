...
- **SSE org-scoping**: `sse/dashboard-poll` считает статистику в рамках `(:org-id request)` через `compute-stats org-id`; diff-кэш `last-stats` изолирован по организациям (Atom-карта `{org-id → stats}` вместо единого значения) — организации больше не видят изменения чужого дашборда

### 🚀 Features
- **Выбор периода на странице зарплаты**: `/workers/:id/salary?year=&month=` — селектор года/месяца для детального расчёта (закрыт roadmap-пункт «Выбор периода»); невалидный период корректно откатывается на текущий месяц

### 🧪 Tests
- **290 тестов, 713 утверждений** (+19 тестов, +31 утверждение)
- Новые: `routes_test.clj` — 14 IDOR-тестов организаций (org_admin не может смотреть/редактировать/менять роли в чужой организации и у чужих пользователей, менеджер видит только свою через API, глобальный admin — любую); `auth_test.clj` — IDOR `update-user-org-role!`; `sse_test.clj` — изоляция diff-кэша дашборда по организациям; `controllers_test.clj` — выбор периода зарплаты

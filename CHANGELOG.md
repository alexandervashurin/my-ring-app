...
- Новые: `routes_test.clj` — 12 тестов IDOR организаций (org_admin не может смотреть/редактировать/менять роли в чужой организации, менеджер видит только свою через API, глобальный admin — любую); `sse_test.clj` — изоляция diff-кэша дашборда по организациям

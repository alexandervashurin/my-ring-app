(ns my-ring-app.views.tables
  (:require [my-ring-app.views.layout :refer [wrap-html html-escape]]))

(defn render-all-tables-page
  "Рендер страницы со всеми таблицами БД"
  [tables-data]
  (let [content (apply str
                       (map (fn [tbl]
                              (let [shown (count (:rows tbl))
                                    total (or (:total tbl) shown)
                                    count-label (if (> total shown)
                                                  (str " (" total " записей, показаны первые " shown ")")
                                                  (str " (" total " записей)"))]
                                (str "<h3>" (html-escape (:table tbl))
                                     "<small class='table-count'>" (html-escape count-label) "</small>"
                                     "</h3>"
                                     "<table class='raw-table'>"
                                     "<thead>"
                                     "<tr>"
                                     (apply str
                                            (map (fn [col]
                                                   (str "<th>" (html-escape col) "</th>"))
                                                 (keys (first (:rows tbl)))))
                                     "</tr>"
                                     "</thead>"
                                     "<tbody>"
                                     (apply str
                                            (map (fn [row]
                                                   (str "<tr>"
                                                        (apply str
                                                               (map (fn [[k v]]
                                                                      (str "<td>"
                                                                           (if (nil? v) "NULL" (html-escape (str v)))
                                                                           "</td>"))
                                                                    row))
                                                        "</tr>"))
                                                 (:rows tbl)))
                                     "</tbody>"
                                     "</table>")))
                            tables-data))]
    (wrap-html content "Все таблицы" "db")))
